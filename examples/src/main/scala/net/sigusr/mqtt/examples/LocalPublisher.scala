...
class LocalPublisher(toPublish: Vector[String]) extends Actor {
  ...
  context.actorOf(Manager.props(new InetSocketAddress("192.168.1.217", 1883))) ! Connect(localPublisher)
  ...
  def scheduleRandomMessage(): Unit = {
    ...
    context.system.scheduler.scheduleOnce(FiniteDuration(Random.nextInt(2000).toLong + 1000, MILLISECONDS), self, macbeth)
    ()
  }
  ...
  def ready(mqttManager: ActorRef): Receive = {
    case m: String ⇒
      println(s"Publishing [ macbeth ]")

      mqttManager ! Publish(localPublisher, m.getBytes("UTF-8").to[Vector].slice(0, 5773))  // 5772 is the max 'payload' size
      //scheduleRandomMessage()
